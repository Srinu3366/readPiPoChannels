function IdocChannelToExcel {
    ...
    foreach ($channel in $channels) {
        if ($i -eq 1) {
            ...
            $excelSheet.Cells($i, 5) = 'ResourceAdapterName'
            $excelSheet.Cells($i,5).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 6) = 'MultipleIdocsInIdocXML'
            $excelSheet.Cells($i,6).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 7) = 'NumberOfIdocsInIdocXML'
        ...
        $excelSheet.Cells($i, 5) = $channel.ResourceAdapterName
        $excelSheet.Cells($i, 6) = $channel.MultipleIdocsInIdocXML
        $excelSheet.Cells($i, 7) = $channel.NumberOfIdocsInXml
        $i++
    }
    return $excelSheet
}