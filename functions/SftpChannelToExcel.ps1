function SftpChannelToExcel {
    ...
    foreach( $channel in $channels ){
        if ($i -eq 1) {
            ...
            $excelSheet.Cells($i, 6) = 'HostName'
            $excelSheet.Cells($i,6).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 7) = 'Port'
            $excelSheet.Cells($i,7).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 8) = 'AuthenticationMethod'
            $excelSheet.Cells($i,8).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 9) = 'UserName'
            $excelSheet.Cells($i,9).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 10) = 'FilePath'
            $excelSheet.Cells($i,10).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 11) = 'FileName'
            $excelSheet.Cells($i,11).Interior.ColorIndex = 16
            $excelSheet.Cells($i, 12) = 'FileDirectory'
            $excelSheet.Cells($i,12).Interior.ColorIndex = 16
            $i++
        }
        ...
        $excelSheet.Cells($i, 6) = $channel.HostName
        $excelSheet.Cells($i, 7) = $channel.Port
        $excelSheet.Cells($i, 8) = $channel.AuthenticationMethod
        $excelSheet.Cells($i, 9) = $channel.UserName
        $excelSheet.Cells($i, 10) = $channel.FilePath
        $excelSheet.Cells($i, 11) = $channel.FileName
        $excelSheet.Cells($i, 12) = $channel.FileDirectory
        $i++    
    }
    return $excelSheet
}